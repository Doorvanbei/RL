...
author: doorvanbei
original paper: How to Learn a Useful Critic? Model-based Action-Gradient-Estimator Policy Optimization (NeurIPS 2020)
date: 20210526
'''
def relu(x):
...
for epoch in range(24):
    ...
    # test
    if (epoch+1) % 3 == 0:
        state = np.array([[np.random.uniform(-np.pi, np.pi)], [np.random.uniform(-8, 8)]])
        for i in range(500):
            x1 = relu(aw1 @ state + ab1)
            x2 = relu(aw2 @ x1 + ab2)
            action = tan2(aw3 @ x2 + ab3)
